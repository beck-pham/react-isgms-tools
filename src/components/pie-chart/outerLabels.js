...
class OuterLabels {
  ...
  resolveDataset(dataset) {
    ...
    if (this.points.length < this.chart.config.data.labels.length) {
      if (this.config.debug) {
        console.error(
          'Too many labels to fit into the available anchor points.'
        );
      }
      return;
    }
  ...
  intersectCircleLine(circle, line) {
    ...
    c = Math.sqrt(
      a * a -
        2 * b * (v2.x * v2.x + v2.y * v2.y - circle.radius * circle.radius)
    );
    if (isNaN(c)) {
      // No intercept
      return [];
  ...
  closest(arr, goal) {
    ...
    return filtered.reduce((prev, curr) => {
      return Math.abs(curr.angle - goal) < Math.abs(prev.angle - goal)
        ? curr
        : prev;
    });
  }
...
/****** Make Space between legend and Chart ******/
